...
#include <inttypes.h>//provids portabl integrs

#define PRT_MFA 4444//Porto No
...
int nxt_client;//var for servr seletn
...
int srvr_selectn() {
    ...
    if (client_counter < 3) {
        nxt_client = 1; // Server
        printf("Next client will be connected to: PRIMARY Server\n");
    } else if (client_counter < 6) {
        nxt_client = 0; // Mirror 1
        printf("Next client will be connected to: Mirror Server 1\n");
    } else if (client_counter < 9) {
        nxt_client = 2; // Mirror 2
        printf("Next client will be connected to: Mirror Server 2\n");
    } else {
        // For remaining connections, alternate between server, mirror1, and mirror2
        if (client_counter % 3 == 0) {
            nxt_client = 1; // Server
            printf("Next client will be connected to: PRIMARY Server\n");
        } else if (client_counter % 3 == 1) {
            nxt_client = 0; // Mirror 1
            printf("Next client will be connected to: Mirror Server 1\n");
        } else {
            nxt_client = 2; // Mirror 2
...
int main(){//Strt Prgm
	...
	if (sckt_fil_dscptr < 0){//chk if it wrks
		printf("connction errorrr.\n");//prnt mssg with err
		exit(1);//destry prm
	}
	printf("serverrr created!!!\n");//prnt mssg with suces
	...
	if (setsockopt(sckt_fil_dscptr, SOL_SOCKET, SO_REUSEADDR, &optn, sizeof(optn))){//set scket optns
        perror("connectionnn Failed");//prnt err
		exit(1);//detry prgm
	}
	...
	while (1){//infi loop
		...
		printf("%d. Connection Accepted  %s:%d", ttl_clients + 1, inet_ntoa(nv_addrss.sin_addr), ntohs(nv_addrss.sin_port));//prnt clint ip and prt no
		int slectd_srver = srvr_selectn();//chose servr type
		if (slectd_srver){//conditn to chk which servr
			prmry_srvr_client_cnt++;//incre prmy val
			write(nv_scket, "P", 1);//wrte val
			ttl_clients++;//incre ttl clints
			printf("\n");//prnt nv lin
		}
		else{//exe wen conditn fail
			mrror_srvr_client_cnt++;//incre  mir servr
			write(nv_scket, "M", 1);//wrte val
			close(nv_scket);//cls sockt
			ttl_clients++;//incre ttl clints
			//printf(" server redirection to Mirrror server\n");//prnt mssg to redrt
			// Debug statement indctng rdrctn to mrr srvr
			printf(" server redirected to Mirror server (Mirror %d)\n", (nxt_client == 2) ? 2 : 1);

			continue;//skp the loop
		}
